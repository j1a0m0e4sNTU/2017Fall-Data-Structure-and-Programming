...
CmdParser::openDofile(const string& dof)
{
   ...
   if(_dofile->is_open()){
     return true;
   }else if(_dofileStack.size()>0){
     cout<< "Error: cannot open file \"" << dof << "\"!!" << endl;
...
CmdParser::parseCmd(string& option)
{
   ...
   if(end== string::npos) option="";
   else option = str.substr(end);
   if(getCmd(str)==0){
...
CmdParser::listCmd(const string& str)
{
   // TODO...
   char* c;
   size_t count,need,begin,end;
   map<const string, CmdExec*>::const_iterator it;
   string cmd="",compare="",input="";
   bool done=false;
   // 1. done
   bool empty=true;
   count =1;
   for(c=_readBuf;c!=_readBufPtr;c++){if(*c!=' ')empty=false;}
   if(empty||_readBufPtr==_readBuf){
     cout<<endl;
     for(it=_cmdMap.begin();it!=_cmdMap.end();it++,count++){
       cmd=it->first+it->second->getOptCmd();
       cout << setw(12) << left << cmd<<flush;
       if(count%5==0)cout<<endl;
     }reprintCmd();cmd="";done=true;
   }
   //2.done  3.done  4.done
   for(c=_readBuf;c!=_readBufPtr;c++){ input+=*c; }
   begin=input.find_first_not_of(' ',0);
   bool check=true;
   for(size_t i=begin;i<input.size();i++){
     if(input.at(i)==' ')check=false;
    }
   if(done==false&&check){
    for(c=_readBuf;c!=_readBufPtr;c++){
      if(*c==' ')continue;
      compare+= *c;
    }
    vector<string> temp;
    for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
     cmd=it->first+it->second->getOptCmd();
     need=compare.size();
     if(need>cmd.size())continue;
     if(myStrNCmp(cmd,compare,need)==0)temp.push_back(cmd);
    }
    if (temp.size()>1){   //2.
      cout<<endl;
      for(size_t i=1;i<=temp.size();i++){
        cout << setw(12) << left <<temp[i-1]<<flush;
        if(i==temp.size())break;
        if(i%5==0)cout<<endl;
      }reprintCmd();done=true;
    }if(temp.size()==1&&*(_readBufPtr-1)!=' '){  //3.
     _tabPressCount = 0;
     for(size_t i=compare.size();i<temp[0].size();i++){
       insertChar(temp[0].at(i));
     } insertChar(' ');
       done=true;
    }if(temp.size()==0){ mybeep();done=true;} //4.
   }
  //5.done
  if(done==false&&_tabPressCount==1){
    input="";
    for(c=_readBuf;c!=_readBufPtr;c++){input += *c;}
    myStrGetTok(input,compare);
    if(compare.size()!=0){
      for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
        cmd=it->first+it->second->getOptCmd();
        need=it->first.size();
        if(myStrNCmp(cmd,compare,need)==0){
          cout<<endl;
  ...
  //7. done
  if(done==false){
    bool check_1=false,check_2=true;
    for(c=_readBuf;c!=_readBufPtr;c++){input += *c;}
    for(int i=input.size()-1;i>=1;i--){
      if(input.at(i)==' '&&input.at(i-1)!=' '){ check_1=true; }
    }
    myStrGetTok(input,compare);
    for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
      cmd=it->first+it->second->getOptCmd();
      need=it->first.size();
      if(myStrNCmp(cmd,compare,need)==0){check_2=false;}
    }if(check_1&&check_2){ mybeep(); done=true;}
  }
  //6.
  vector<string> files;
  if(done==false&&_tabPressCount>1){
    if(*(_readBufPtr-1)==' '){
      ...
      for(size_t i=0;i<files.size();i++){
        if( i%5 == 0)cout<<endl;
        cout << setw(16) << left <<files[i]<<flush;
      }reprintCmd();
    }else{
    compare="";
    input="";
    ...
    for(size_t i=end+1;i<input.size();i++)compare+= input.at(i);
    listDir(files,compare,".");
    if(files.size()>1){
        string temp="";
        bool common=true;
        for(size_t i=compare.size();i<files[0].size()&&common==true;i++){
          for(size_t j=1;j<files.size();j++){
            if(files[0].at(i)!=files[j].at(i)){common=false;break;}
          }if(common)temp+= files[0].at(i);
        }
        if(temp.size()>0){
          for(size_t i=0;i<temp.size();i++)insertChar(temp.at(i));
        }else{
          cout<<endl;
          for(size_t i=0;i<files.size();i++){
            if( i%5 == 0)cout<<endl;
      ...
      for(size_t i=compare.size();i<files[0].size();i++){insertChar(files[0].at(i));}
      insertChar(' ');
    }else mybeep();
  }
 }  
...
CmdParser::getCmd(string cmd)
{
   ...
   // TODO...
   string command ;
   myStrGetTok(cmd,command);
   string EXE="";
   unsigned n;
   map<const string, CmdExec*>::const_iterator it;
   for(it=_cmdMap.begin();it!=_cmdMap.end();it++){
     EXE=it->first+it->second->getOptCmd();
     n= it->first.size();
     if(myStrNCmp(EXE,command,n)==0){ return it->second ; }
     EXE="";
   }
   return e;
}
